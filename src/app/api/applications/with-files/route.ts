  ...
  try {
    ...
    if (!contentType || !contentType.includes("multipart/form-data")) {
      ...
      logError(
        ...
        new Error(`Expected multipart/form-data, got: ${contentType}`),
        {
          userId: session.user.id,
          contentType,
        }
      ...
      return NextResponse.json(
        {
          error: `Invalid content type. Expected multipart/form-data, got: ${
            contentType || "none"
          }`,
        },
        { status: 400 }
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Failed to parse form data. Please ensure you are sending valid multipart/form-data",
        },
        { status: 400 }
      );
    }
    ...
    if (!applicationDataStr) {
      ...
      logError(
        ...
        new Error("applicationData field is required"),
        { userId: session.user.id }
      );
      logApiResponse(method, url, 400, duration, session.user.id);
      return NextResponse.json(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      logError("Invalid application data JSON", error, {
        userId: session.user.id,
      });
      logApiResponse(method, url, 400, duration, session.user.id);
      return NextResponse.json(
    ...
    const mappedApplicationData = {
      ...
      personalDetails: {
        ...
        employment: {
          ...
          workExperience: parseWorkExperience(
            applicationData.financialInfo.workExperience
          ),
        },
        contact: {
      ...
      loanDetails: {
        ...
        emi: Math.round(
          (applicationData.loanInfo.amount * 0.12 * Math.pow(1.12, 1)) /
            (Math.pow(1.12, 1) - 1) /
            12
    ...
    logInfo("Loan application created", {
      ...
      for (const file of files) {
        if (file && file.size > 0) {
          try {
            ...
            console.log(file.name, file.type, fieldName);
    ...
    if (uploadedFiles.length > 0) {
      savedApplication.documents = uploadedFiles.map((file) => ({
        fileUrl: file.fileUrl,
        fileName: file.fileName,
        documentType: file.documentType,
        uploadedAt: file.uploadedAt,
        status: "uploaded", // or whatever default status
      }));
